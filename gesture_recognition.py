...
center_zone_area = cv2.imread("zone_area.jpeg")
img_pointing = cv2.imread("image_pointing.jpeg")
...
if center_zone_area is not None:
    center_zone_area = cv2.resize(center_zone_area, (WINDOW_WIDTH, WINDOW_HEIGHT))
...
def draw_detection_zone(frame):
    """Draw the Center detection zone on the frame"""
...
def is_pointing_gesture(hand_landmarks):
    ...
    )  # Tip is "above" PIP in y-coordinate (screen Y-axis is inverted)
...
while cap.isOpened():
    ...
    # Draw the detection zone for finger-in-center-area gesture
    ...
    if smooth_detected == "finger in center area" and center_zone_area is not None:
        gesture_display = center_zone_area.copy()
    ...
    status_text = (
        "No gesture" if detected_gesture is None else f"Detected: {detected_gesture}"
    )
    cv2.putText(
        camera_display,
        status_text,
